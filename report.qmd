---
title: "S&DS 425/625 Report: Landcover Classification"
date: "`r Sys.Date()`"
...
This report evaluates multiple machine learning approaches for land cover classification using satellite-derived spectral, temporal, and spatial predictors. Models include Multinomial Logistic Regression (baseline), a temporal extension through Sinusoidal Regression, the non-linear CART algorithm, and the ensemble-based Random Forest model. While logistic regression provided interpretability, it struggled with class imbalance and complex feature interactions. Sinusoidal regression captured periodic trends but offered minimal improvement. The CART model addressed non-linear relationships but overfit the data, particularly for spectrally similar classes. Random Forest emerged as the best-performing model, achieving a macro-averaged F1-score of 0.812 and balanced accuracy of 0.877, demonstrating its ability to handle class imbalance and capture complex patterns. Key predictors included NDVI, Vegetation Indicators, and Elevation. The study underscores the importance of non-linear models and ensemble methods for accurate and robust land cover classification.
...
CART forms the foundation of more advanced algorithms like Random Forest. The CART algorithm builds a *single* decision tree by recursively partitioning the feature space to predict the class of the target variable. At each split, the algorithm selects the optimal feature and threshold by minimizing an impurity criterion (e.g., Entropy measuring the uncertainty of a node or Gini Index measuring the probability of misclassification). It stops when a pre-defined criterion is met (e.g. minimum number of samples in a node, maximum tree depth, or impurity improvement below a threshold). Lastly, it directly assigns a class label to the leaf nodes based on the majority vote in those nodes (for classification).
...
The dataset was filtered to points within U.S. borders and assessed in year-wise batches, with unique entries defined by plotid. Landcover classes were dynamically encoded, and visualized on the [Google Earth Engine](https://earthengine.google.com) interface:

![Map of Visualized Data](sims/actual_map.png)

The dataset was divided using a 70%-30% training-testing split. Thus, the CART model was trained on 70% of the dataset and tested on 30% of the dataset, using predictors: `NDVI`, `SR_B1`, `SR_B2`, `SR_B3`, `SR_B4`, `SR_B5`, `SR_B7`, `elevation_meters` (See full code [here](https://code.earthengine.google.com/27be438d2822e76c39af6aece7d61aad)).
...
The CART model was evaluated using accuracy, precision, recall, and the kappa ($\kappa$) coefficient. It yielded an accuracy of 55.3% and a $\kappa$ coefficient of 0.304.
...
A rectangular region is defined around NY and expanded into a 60 kilometer buffer, from which a “doughnut” buffer is created by excluding the original rectangle. Points within the rectangle are used as training data, while points within the doughnut buffer are designated as test data. A Landsat 8 composite image is generated for the region, and predictor bands (B1–B7) are sampled at training points to train a CART classifier. The classifier is then applied to the composite image to produce a classified landcover map. The classification results are validated using test points, and a confusion matrix is computed to evaluate performance, including overall accuracy and the Kappa coefficient. Misclassified points, identified by comparing predicted values (classification) to true values (landcover), are highlighted in red and displayed on the map.

![Map of Classified Landcover in New York](sims/mapoverlays.png)

### Performance Evaluation

Using the full dataset for the region of interest (New York), the CART model achieved an overall accuracy of 73.84%, and a kappa coefficient of 0.52. This indicates a significant improvement relative to the previous CART model. The classifier incorrectly classified 72 out of 241 test observations, corresponding to a 29% misclassification rate. The confusion matrix shows that the model still struggles to distinguish between certain classes, such as "Trees" and "Grass/Forb/Herb", which are often misclassified as one another. It should be noted that "Snow/Ice" was not included in the training set because no "Snow/Ice" labels existed in NY.
...
This section details the implementation of multiple classification models, including logistic regression (linear baseline), sinusoidal regression (temporal modeling), CART algorithm (non-linear), and Random Forests (non-linear, ensemble). Each model addresses specific challenges identified in the data exploration phase, such as overlapping class boundaries, temporal periodicity, and spatial dependencies.

# Conclusions and Recommendations {#conclusions-recs}

This report presents a comprehensive analysis of land cover classification using multiple modeling approaches, including Multinomial Logistic Regression, a temporal extension through Sinusoidal Regression, the CART algorithm, and the ensemble-based Random Forest model. Each method was systematically evaluated to address specific challenges inherent in remote sensing data, including class imbalance, non-linear relationships, overlapping class boundaries, and temporal-spatial dependencies.

The Multinomial Logistic Regression served as a baseline model, offering interpretability and highlighting the importance of predictors such as NDVI, spectral bands, and spatial indicators. While effective for dominant classes like "Water" and "Grass/Forb/Herb", the model struggled with underrepresented classes such as "Shrubs" and "Snow/Ice", resulting in poor recall and precision for these categories. The sinusoidal extension introduced periodicity through Fourier-transformed sine and cosine terms, capturing seasonal trends, but failed to deliver substantial improvements, suggesting that periodic components provided limited additional predictive power.

The CART algorithm demonstrated the advantages of non-linear models by accounting for complex interactions among predictors. However, its reliance on a single decision tree led to overfitting, particularly for classes with similar spectral signatures such as "Trees" and "Grass/Forb/Herb". Misclassification rates were notably high for minority classes like "Shrubs" and "Snow/Ice", reflecting the challenges of class imbalance in the training data. Geospatial analysis further revealed regional misclassification patterns, particularly in vegetation-dominated areas, underscoring the need for enhanced feature differentiation.

The Random Forest model emerged as the best-performing approach, achieving significant improvements in accuracy, precision, and recall. By aggregating predictions from multiple decision trees, Random Forests effectively addressed the limitations of CART, reducing overfitting and capturing complex relationships in the data. With a macro-averaged F1-score of 0.812 and balanced accuracy of 0.877, the Random Forest model demonstrated superior performance across all classes, including substantial gains for minority categories like "Snow/Ice". Variable importance analysis further identified NDVI, Vegetation Indicators, and Elevation as key predictors, reinforcing their ecological relevance in distinguishing land cover types.

To improve our exploration, many factors could be considered. First, enhancing class representation by oversampling minority classes (e.g., "Shrubs", "Snow/Ice") or undersampling dominant ones could improve model performance.Second, incorporating additional features like texture metrics, vegetation indices, or spatial aggregates could improve class separability. Third, prioritizing ensemble methods like Random Forests or Gradient Boosting is vital for future land cover classification tasks due to their flexibility and robustness.Finally, incorporating spatial relationships explicitly through neighborhood-level statistics or spatial smoothing techniques could account for ecological continuity.

In conclusion, this study highlights the importance of integrating spectral, temporal, and spatial dimensions into land cover modeling. While linear models offer clarity and baseline performance, advanced non-linear approaches like Random Forests provide the accuracy and robustness required for real-world environmental monitoring.
